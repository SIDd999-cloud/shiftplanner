...
// ── Skill Picker ─────────────────────────────────────────────────────────────
function SkillPicker({ skills, onChange, allSkills }: { skills: string[]; onChange: (s: string[]) => void; allSkills: string[] }) {
...
// ── Form Fields ───────────────────────────────────────────────────────────────
function FormFields({ f, setF, allSkills }: { f: PersonForm; setF: (x: PersonForm) => void; allSkills: string[] }) {
  return (
    <div className="space-y-3">
      ...
      <SkillPicker skills={f.skills} onChange={s => setF({ ...f, skills: s })} allSkills={allSkills} />
    </div>
  )
}
  ...
  function PersonCard({ person }: { person: Person }) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        {editingId === person.id ? (
          <div>
            <FormFields f={editForm} setF={setEditForm} allSkills={allSkills} />
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 mb-6 space-y-3">
        ...
        <FormFields f={form} setF={setForm} allSkills={allSkills} />
